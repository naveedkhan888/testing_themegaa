<?php if (lasa_tbay_get_config('show_searchform')): ?>

	<?php
        $_id = lasa_tbay_random_key();

        $autocomplete_search 	=  lasa_tbay_get_config('autocomplete_search', true);

        $enable_image 	=  lasa_tbay_get_config('show_search_product_image', true);
        $enable_price 	=  lasa_tbay_get_config('show_search_product_price', true);
        $search_type 	=  lasa_tbay_get_config('search_type', 'product');
        $number 		=  lasa_tbay_get_config('search_max_number_results', 5);
        $minchars 		=  lasa_tbay_get_config('search_min_chars', 3);

        $search_text_categories 		=  lasa_tbay_get_config('search_text_categories', esc_html__('All categories', 'lasa'));
        $search_placeholder 			=  lasa_tbay_get_config('search_placeholder', esc_html__('I&rsquo;m shopping for...', 'lasa'));
        $button_search 				=  lasa_tbay_get_config('button_search', 'all');
        $button_search_text 		=  lasa_tbay_get_config('button_search_text', 'Search');
        $button_search_icon 		=  lasa_tbay_get_config('button_search_icon', 'tb-icon tb-icon-magnifier');


        $show_count 					= lasa_tbay_get_config('search_count_categories', false);
	...
	<div class="tbay-search-form">
			...
			<div class="form-group">
				<div class="input-group">
					<?php if (lasa_tbay_get_config('search_category')): ?>
						<?php
                            wp_enqueue_style('sumoselect');
                            wp_enqueue_script('jquery-sumoselect');
                        ?>
						<div class="select-category input-group-addon">
							<?php if (class_exists('WooCommerce') && lasa_tbay_get_config('search_type') == 'product') :
                                $args = array(
							...
							<?php elseif (lasa_tbay_get_config('search_type') == 'post'):
                                $args = array(
				  	...
				  	<?php endif; ?>
						...
						<input type="hidden" name="post_type" value="<?php echo esc_attr(lasa_tbay_get_config('search_type')); ?>" class="post_type" />
				</div>
