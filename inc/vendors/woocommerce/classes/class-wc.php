...
if (! class_exists('Lasa_WooCommerce')) :


    class Lasa_WooCommerce
    {
        public static $instance;

        public static function getInstance()
        {
            if (! isset(self::$instance) && ! (self::$instance instanceof Lasa_WooCommerce)) {
                self::$instance = new Lasa_WooCommerce();
            }
        ...
        private function init_hooks()
        {
            ...
            add_filter('lasa_woo_pro_des_image', array( $this, 'shop_des_image_active'), 10, 1);
            ...
            add_action( 'lasa_woocommerce_after_product_thumbnails', array( $this, 'quick_view_view_details_btn'), 10 );

            add_filter('lasa_tbay_woocommerce_content_class', array( $this, 'woocommerce_content_class'), 10);
            ...
            add_action('lasa_woocommerce_before_quick_view', array( $this, 'remove_quickview_hook'), 10);
            add_action('lasa_woocommerce_after_quick_view', array( $this, 'add_quickview_hook'), 10);

            add_action('lasa_tbay_after_shop_loop_item_title', array( $this, 'catalog_mode_remove_shop_loop_item_hook'), 10);
            ...
            add_filter('lasa_enable_variation_selector', array( $this, 'enable_variation_selector'), 10);


            /*Show Quantity on mobile*/
            add_filter('lasa_show_quantity_mobile', array( $this, 'show_quantity_mobile'), 10, 1);
            ...
            if ( lasa_nextend_social_login_activated() ) {
            ...
            add_filter('lasa_get_filter_title_mobile', array( $this, 'get_title_mobile'), 10, 1);
        ...
        public function woocommerce_scripts()
        {
            ...
            wp_enqueue_script('lasa-woocommerce', LASA_SCRIPTS . '/woocommerce' . $suffix . '.js', array( 'lasa-script' ), LASA_THEME_VERSION, true);

            wp_register_script('jquery-onepagenav', LASA_SCRIPTS . '/jquery.onepagenav' . $suffix . '.js', array( 'lasa-script' ), '3.0.0', true);
        }
        ...
        public function compare_styles()
        {
            ...
            wp_enqueue_style('lasa-font-tbay-custom');

            lasa_tbay_fonts_url();

            wp_enqueue_style('lasa-template');
            
            wp_enqueue_style('lasa-skin');
        ...
        public function body_woo_class($classes)
        {
            ...
            $class  = lasa_add_cssclass('woocommerce', $class);

            if (is_cart() && WC()->cart->is_empty()) {
                $class = lasa_add_cssclass('empty-cart', $class);
            }
        ...
        public function widgets_init()
        {
            register_sidebar(array(
                'name'          => esc_html__('Product Archive Sidebar Top', 'lasa'),
                'id'            => 'product-top-archive',
                'description'   => esc_html__('Add widgets here to appear in only shop page.', 'lasa'),
            ...
            register_sidebar(array(
                'name'          => esc_html__('Product Archive Sidebar Bottom', 'lasa'),
                'id'            => 'product-bottom-archive',
                'description'   => esc_html__('Add widgets here to appear in bottom product archive.', 'lasa'),
                'before_widget' => '<aside id="%1$s" class="widget %2$s">',
                'after_widget'  => '</aside>',
                'before_title'  => '<h2 class="widget-title">',
                'after_title'   => '</h2>',
            ...
            register_sidebar(array(
                'name'          => esc_html__('Product Archive Sidebar', 'lasa'),
                'id'            => 'product-archive',
                'description'   => esc_html__('Add widgets here to appear in Product archive left, right sidebar.', 'lasa'),
                'before_widget' => '<aside id="%1$s" class="widget %2$s">',
                'after_widget'  => '</aside>',
                'before_title'  => '<h2 class="widget-title">',
                'after_title'   => '</h2>',
            ...
            register_sidebar(array(
                'name'          => esc_html__('Product Single Sidebar', 'lasa'),
                'id'            => 'product-single',
                'description'   => esc_html__('Add widgets here to appear in Product single left, right sidebar.', 'lasa'),
                'before_widget' => '<aside id="%1$s" class="widget %2$s">',
                'after_widget'  => '</aside>',
                'before_title'  => '<h2 class="widget-title">',
                'after_title'   => '</h2>',
        ...
        public function woocommerce_content_class($class)
        {
            ...
            if (!isset($_GET['product_'.$page.'_layout'])) {
                $class .= ' '.lasa_tbay_get_config('product_'.$page.'_layout');
            } else {
        ...
        {
            $class = '';
            $active = lasa_catalog_mode_active();
        ...
        public function remove_quickview_hook()
        {
            ...
            remove_action('woocommerce_after_add_to_cart_form', 'lasa_product_popup_group_buttons', 35);

            /*Add custom html before, after button add to cart*/
            remove_action('woocommerce_before_add_to_cart_form', 'lasa_html_before_add_to_cart_button', 10);
            remove_action('woocommerce_after_add_to_cart_form', 'lasa_html_after_add_to_cart_button', 99);

            /*Add custom html before, after inner product summary*/
            remove_action('woocommerce_single_product_summary', 'lasa_html_before_inner_product_summary', 1);
            remove_action('woocommerce_single_product_summary', 'lasa_html_after_inner_product_summary', 99);
            remove_action('lasa_woocommerce_single_product_summary_left', 'lasa_html_after_inner_product_summary', 99);

            remove_action('woocommerce_before_single_product', 'lasa_html_before_product_summary', 5);
            remove_action('woocommerce_after_single_product', 'lasa_html_after_product_summary', 5);
        }

        public function add_quickview_hook()
        {
            add_action( 'woocommerce_after_single_product_summary', 'woocommerce_output_related_products', 20 );
            add_action('woocommerce_after_add_to_cart_form', 'lasa_product_popup_group_buttons', 35);
            ...
            if ( class_exists('YITH_WFBT_Frontend') ) {
                add_action( 'woocommerce_after_single_product_summary', array( YITH_WFBT_Frontend::get_instance(), 'add_bought_together_form' ), 1 );
                        ...
                        /*Add custom html before, after button add to cart*/
            add_action('woocommerce_before_add_to_cart_form', 'lasa_html_before_add_to_cart_button', 10, 0);
            add_action('woocommerce_after_add_to_cart_form', 'lasa_html_after_add_to_cart_button', 99);

            /*Add custom html before, after inner product summary*/
            add_action('woocommerce_single_product_summary', 'lasa_html_before_inner_product_summary', 1, 0);
            add_action('woocommerce_single_product_summary', 'lasa_html_after_inner_product_summary', 99);
            add_action('lasa_woocommerce_single_product_summary_left', 'lasa_html_after_inner_product_summary', 99);

            add_action('woocommerce_before_single_product', 'lasa_html_before_product_summary', 5, 0);
            add_action('woocommerce_after_single_product', 'lasa_html_after_product_summary', 5, 0);
        }
        ...
        public function add_compare_styles()
        {
            ...
            wp_enqueue_style( 'lasa-style' );
        }

        public function enable_variation_selector()
        {
            $active = lasa_tbay_get_config('enable_variation_swatch', false);
        ...
        {
            $active = lasa_tbay_get_config('enable_quantity_mobile', false);
        ...
        {
            $class = '';
            $active = apply_filters('lasa_show_quantity_mobile', 10, 2);
            if (isset($active) && $active) {
                $class = 'tbmb-quantity';
            }
        ...
        {
            $active = lasa_tbay_get_config('show_woocommerce_password_strength', true);
        ...
        public function yith_add_wcwl_link_my_account($items)
        {
            ...
            $items['wishlist']                  =   esc_html__('My Wishlist', 'lasa');
            $items['edit-address']              =   esc_html__('Addresses', 'lasa');
            $items['payment-methods']           =   esc_html__('Payment methods', 'lasa');
            $items['edit-account']              =   esc_html__('Account details', 'lasa');
            $items['customer-logout']           =   esc_html__('Logout', 'lasa');
        ...
        {
            $format                 =  lasa_tbay_get_config('sale_tags', 'custom');
            $enable_label_featured  =  lasa_tbay_get_config('enable_label_featured', true);

            if ($format == 'custom') {
                $format = lasa_tbay_get_config('sale_tag_custom', '-{percent-diff}%');
            }
            ...
            if ($featured && $enable_label_featured) {
                $return_content  = '<span class="featured">'. lasa_tbay_get_config('custom_label_featured', esc_html__('Hot', 'lasa')) .'</span>';
            }
            ...
            if (!empty($product) && $product->is_type('variable')) {
                ...
                if (!empty($variation_id)) {
                    ...
                    $percentage = '<span class="saled">'. esc_html__('Sale', 'lasa') . '</span>';
                }
        ...
        public function only_feature_product_label()
        {
            ...
            if ($featured) {
                ...
                if ($featured && $enable_label_featured) {
                    $return_content  .= '<span class="only-featured onsale"><span class="featured">'. lasa_tbay_get_config('custom_label_featured', esc_html__('Hot', 'lasa')) .'</span></span>';
        ...
        public function quick_view_view_details_btn( ) {
			...
			echo '<div class="details-btn-wrapper"><a class="view-details-btn" href="'. esc_url($permalink) .'">'. esc_html__('View details', 'lasa') .'</a></div>';
        }
        ...
        {
            if ( lasa_nextend_social_login_activated() ) {
                echo '<div class="social-log"><span>'. esc_html__('Or connect with', 'lasa') .'</span></div>';
            }
        }
        ...
        {
            if ( lasa_nextend_social_login_activated() ) {
                echo '<div class="social-log"><span>'. esc_html__('Or login with', 'lasa') .'</span></div>';
            }
        }
        ...
        public function show_product_outstock_flash_html($html)
        {
            ...
            if ($product->is_type('simple')) {
                if ($product->is_on_sale() &&  ! $product->is_in_stock()) {
                    $return_content .= '<span class="out-stock out-stock-sale"><span>'. esc_html__('Sold Out', 'lasa') .'</span></span>';
                } elseif (! $product->is_in_stock()) {
                    $return_content .= '<span class="out-stock"><span>' . esc_html__('Sold Out', 'lasa') .'</span></span>';
                }
            }
        ...
        {
            $columns = lasa_tbay_get_config('number_product_thumbnail', 4);
        ...
        {
            $pagination_style = (isset($_GET['pagination_style'])) ? $_GET['pagination_style'] : lasa_tbay_get_config('product_pagination_style', 'number');
        ...
        public function get_title_mobile($title)
        {
            if (is_author()) {
                ...
                $title = esc_html__('Tagged: "', 'lasa'). single_tag_title('', false) . '"';
        ...
        {
            $active = lasa_tbay_get_config('pro_des_image_product_archives', false);
...
function Lasa_WooCommerce()
{
    return Lasa_WooCommerce::getInstance();
}

// Global for backwards compatibility.
$GLOBALS['Lasa_WooCommerce'] = Lasa_WooCommerce();