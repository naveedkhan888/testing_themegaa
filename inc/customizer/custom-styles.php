...
if (!function_exists('themename_xptheme_color_lightens_darkens')) {
    /**
     ...
     */
    function themename_xptheme_color_lightens_darkens($hex, $percent)
    {
...
if (!function_exists('themename_xptheme_default_theme_primary_color')) {
    function themename_xptheme_default_theme_primary_color()
    {
...
if ( !function_exists ('themename_xptheme_default_theme_primary_fonts') ) {
	function themename_xptheme_default_theme_primary_fonts() {
...
if (!function_exists('themename_xptheme_check_empty_customize')) {
    function themename_xptheme_check_empty_customize($option, $default){
	 	echo !empty($option) ? esc_html($option) : esc_html($default);
	}
}


if (!function_exists('themename_xptheme_theme_primary_color')) {
    function themename_xptheme_theme_primary_color()
    {
		$default_color 		= themename_xptheme_default_theme_primary_color(); 
		$boby_bg   			= themename_xptheme_get_config('boby_bg');


		$main_color   = themename_xptheme_get_config(('main_color'),$default_color['main_color']);

		if( !empty($default_color['enable_main_color_text']) ) {
			$main_color_text   = themename_xptheme_get_config(('main_color_text'),$default_color['main_color_text']);
		}

		$header_mobile_bg   = themename_xptheme_get_config(('header_mobile_bg'),$default_color['header_mobile_bg']);
		$header_mobile_color   = themename_xptheme_get_config(('header_mobile_color'),$default_color['header_mobile_color']);

		

		if( !empty($default_color['btn_text_color']) ) {
			$btn_text_color   = themename_xptheme_get_config(('btn_text_color'),$default_color['btn_text_color']);
		} else {
			$btn_text_color   = themename_xptheme_get_config('btn_text_color');  
		}
		?>
		:root {
...
if (!function_exists('themename_xptheme_custom_styles')) {
    function themename_xptheme_custom_styles()
    {
		ob_start();

		themename_xptheme_theme_primary_color();
		...
		/*End Theme Color*/
		if (themename_redux_framework_activated()) {
			$logo_img_width        		= themename_xptheme_get_config('logo_img_width');
			$logo_padding        		= themename_xptheme_get_config('logo_padding');

			$logo_img_width_mobile 		= themename_xptheme_get_config('logo_img_width_mobile');
			$logo_mobile_padding 		= themename_xptheme_get_config('logo_mobile_padding');

			$checkout_img_width 		= themename_xptheme_get_config('checkout_img_width');

			$custom_css 			= themename_xptheme_get_config('custom_css');
			$css_desktop 			= themename_xptheme_get_config('css_desktop');
			$css_tablet 			= themename_xptheme_get_config('css_tablet');
			$css_wide_mobile 		= themename_xptheme_get_config('css_wide_mobile');
			$css_mobile         	= themename_xptheme_get_config('css_mobile');
			...
			$show_typography        = (bool) themename_xptheme_get_config('show_typography', false);
			
			if ($show_typography) {
                $font_source 			= themename_xptheme_get_config('font_source');

				if ( !empty(themename_xptheme_get_config('main_font')['font-family']) ) {
					$primary_font 			= themename_xptheme_get_config('main_font')['font-family'];
				}
				if ( !empty(themename_xptheme_get_config('main_font_second')['font-family']) ) {
					$second_font					= themename_xptheme_get_config('main_font_second')['font-family'];
				}
                $main_google_font_face = themename_xptheme_get_config('main_google_font_face');
                $main_custom_font_face = themename_xptheme_get_config('main_custom_font_face');
                $main_second_google_font_face 	= themename_xptheme_get_config('main_second_google_font_face');
                $main_second_custom_font_face 	= themename_xptheme_get_config('main_second_custom_font_face');
...
?>
    ...
    add_action('wp_enqueue_scripts', 'themename_xptheme_custom_styles', 200);
}
