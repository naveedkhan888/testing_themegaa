		...
		<tbody>
			...
			foreach ( $grouped_products as $grouped_product_child ) {
				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					switch ( $column_id ) {
                        case 'image': 
                            $value  = '<a href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', $grouped_product_child->get_permalink(), $grouped_product_child->get_id() ) ) . '">' ; 
                            $value  .= wp_get_attachment_image($grouped_product_child->get_image_id(),array('100', '100'));  
                            $value  .= '</a>';
                            break;  
						case 'quantity': 
							ob_start(); 
